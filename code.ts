...
type PrimitiveNodes = Array<
  "RECTANGLE" | "LINE" | "VECTOR" | "ELLIPSE" | "POLYGON" | "STAR"
>;
...
const PRIMITIVES: PrimitiveNodes = [
  "RECTANGLE",
  "LINE",
  "VECTOR",
  "ELLIPSE",
...
function checkName(type: any, newName: string) {
  ...
  if (PRIMITIVES.includes(type)) {
    const re = new RegExp(
      "Rectangles[0-9]+|Lines[0-9]+|Arrows[0-9]+|Ellipses[0-9]+|Polygons[0-9]+|Stars[0-9]+"
    );
    return !re.test(newName);
  }
...
function getDefaultNamedLayers() {
  allNodes = figma.currentPage.findAllWithCriteria({
    types: ["FRAME", "INSTANCE", ...PRIMITIVES],
  });

  const frames: any = allNodes.filter((node: any) => node.type === "FRAME");
  framesCount = frames.length;

  defaultFrames = frames.filter((frame: any) =>
    frame.name.match(/Frame [0-9]+/)
  );
  emptyFrames = frames.filter((frame: any) => frame.children.length === 0);

  const shapes: any = allNodes.filter((node: any) =>
    PRIMITIVES.includes(node.type)
  );
  shapesCount = shapes.length;
  defaultShapes = shapes.filter((shape: any) =>
    shape.name.match(
      /Rectangle\s[0-9]+|Line\s[0-9]+|Arrow\s[0-9]+|Ellipse\s[0-9]+|Polygon\s[0-9]+|Star\s[0-9]+/
    )
  );

  const instances: any = allNodes.filter(
    (node: any) => node.type === "INSTANCE"
  );
  instancesCount = instances.length;
  defaultInstances = instances.filter(
    (node: any) => node.name === node?.mainComponent?.name
  );
...
figma.ui.onmessage = (msg) => {
  ...
  if (action === "FIX_NAMING") {
    ...
    for (const frame of badNamedNodes) {
      if (convention === "1") {
        frame.name = frame.name[0].toLowerCase() + frame.name.slice(1);
      }
      if (convention === "2") {
        frame.name = frame.name[0].toUpperCase() + frame.name.slice(1);
      }
  ...
  if (action === "SELECT_ITEM") {
    ...
    if (newName) {
      const node = figma.currentPage.findOne(
        (node) => node.id === defaultFrames[0].id
      );
      if (node && checkName(node.type, newName)) {
        node.name = msg.newName;
        defaultFrames.shift();
      } else {
        figma.notify("Node is still have a default name", {
          error: true,
          timeout: 3000,
        });
        return;
